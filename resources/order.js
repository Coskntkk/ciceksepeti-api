...
Order.prototype.list = async function list(params) {
    ...
    if ((!params.startDate || !params.endDate) && !params.orderNo && !params.orderItemNo) {
        throw new Error(
            'Start date (startDate) and end date (endDate) are required if order no (orderNo) and order item no (orderItemNo) are not provided.'
        )
    }
    if (params.startDate && params.endDate) {
        ...
        if (diff > 14) {
            throw new Error(
                'The difference between start date (startDate) and end date (endDate) cannot be more than 14 days.'
            )
        }
    }
